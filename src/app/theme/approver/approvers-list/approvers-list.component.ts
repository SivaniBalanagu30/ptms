...
import { Constants, RegExpPatterns, StaticDataEntity } from 'src/app/shared/static-data';
...
export class ApproversListComponent implements OnInit {
    ...
    columnDefs: [
      ...
      { 
        ...
        cellRenderer: (params: any) => {
          ...
          if(params.data?.status === 'Saved'){
            return '<span style= "color: #674ea7;">Draft</span>';
          }
        }
      },
      ...
      // { headerName: 'Resource Name', field: 'resource_name', sortable: true, suppressSizeToFit: true, width: 210 },
      {
        ...
        cellRenderer: (params: any) => {
          if (params.data?.status === 'Submitted') {
            ...
            return "<i class='view mr-1 fa-solid fa-file-lines' style='cursor:pointer; font-size:17px;'  alt='fine-print--v1' data-toggle='tooltip' data-placement='top' title='View timesheets' ></i>"
        ...
        onCellClicked: (params: any) => {
          ...
          if (iconName.includes('fa-file-pdf')) {
  ...
  dataSource: IDatasource = {
    getRows: (params: IGetRowsParams) => {
      ...
      this.httpService.doPost(`adminApproveTimesheets?pageNumber=${Number(this.pageNumber)}&pageSize=${Number(10)}`, payload).subscribe((result) => {
  ...
  doPostRequest(payload: any) {
    return this.httpService.doPost(`adminApproveTimesheets?pageNumber=${Number(this.pageNumber)}&pageSize=${Number(10)}`, payload);
  }
  ...
  showModal(data) {
    this.isCommentsSubmitted = false;
    ...
    this.viewColumnsDefs = [
      ...
      { headerName: 'Work Hours', field: 'work_hour', tooltipField:'work_hour', sortable: true, suppressSizeToFit: true, width: 85 },
  ...
  getProjects() {
    this.httpService.doGet(StaticDataEntity.AdminProjectList).subscribe((result: any) => {
      this.projectsList = result;
    })
  }

  getResouces() {
    this.httpService.doGet(StaticDataEntity.AdminResourceList).subscribe((result: any) => {
